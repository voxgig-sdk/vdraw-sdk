...
class UsernameGenerationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = username_generation_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "username_generation." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set VDRAW_TEST_USERNAME_GENERATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    username_generation_ref01_ent = client.UsernameGeneration(nil)
...
def username_generation_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "username_generation", "UsernameGenerationTestData.json")
  ...
  entid_env_raw = ENV["VDRAW_TEST_USERNAME_GENERATION_ENTID"]
  ...
  if env["VDRAW_TEST_LIVE"] == "TRUE"
    ...
    client = VdrawSDK.new(Helpers.to_map(merged_opts))
  end
