...
class UsernameGenerationEntity
  ...
  def make
    opts = @_entopts.dup
    UsernameGenerationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = VdrawHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [UsernameGeneration, Hash] the current UsernameGeneration data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = VdrawHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of UsernameGeneration fields)
  def match_get
  ...
  #
  # @param reqdata [UsernameGenerationCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [UsernameGeneration, Hash] the created UsernameGeneration; raises VdrawError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = VdrawHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
