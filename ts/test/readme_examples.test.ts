...
//     (\`new VdrawSDK(...)\` / \`VdrawSDK.test(...)\` become
//     \`VdrawSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"username_generation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new VdrawSDK(...)\` and any
// \`VdrawSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+VdrawSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/VdrawSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
