...
import (
	...
	omni "github.com/voxgig-sdk/vdraw-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VdrawError); is {
				out := map[string]any{
					"name":    "VdrawError",
...
func omniCtx(arg any, client *sdk.VdrawSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VdrawSDK); is {
			client = live
