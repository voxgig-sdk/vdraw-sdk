...
{
    private static function utility(): VdrawUtility
    {
        return new VdrawUtility();
    }
    ...
    private static function ctx(array $over = []): VdrawContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new VdrawContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new VdrawOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof VdrawError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new VdrawOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = VdrawMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new VdrawOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = VdrawMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new VdrawOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = VdrawMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VdrawSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = VdrawMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new VdrawResponse([]),
            'result' => new VdrawResult([]),
        ]);
        [, $err] = VdrawMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new VdrawSpec(['step' => 's']),
            'response' => new VdrawResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new VdrawResult([]),
        ]);
        [, $err] = VdrawMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new VdrawSpec(['step' => 's']),
            'response' => new VdrawResponse(self::resp(200, ['v' => 1])),
            'result' => new VdrawResult([]),
        ]);
        [, $err] = VdrawMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new VdrawSpec(['step' => 's']),
            'response' => new VdrawResponse(self::resp(200, ['v' => 2])),
            'result' => new VdrawResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        VdrawMakeResponse::call($ctx);
    ...
    {
        $preset = new VdrawResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new VdrawSpec([]),
            'response' => new VdrawResponse([]),
            'result' => new VdrawResult([]),
        ...
        [$got, $err] = VdrawMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new VdrawResult([])]);
        [, $err] = VdrawMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new VdrawSpec([]), 'result' => null]);
        [, $err] = VdrawMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VdrawSpec(['step' => 's']),
            'result' => new VdrawResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new VdrawOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VdrawMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VdrawSpec(['step' => 's']),
            'result' => new VdrawResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new VdrawOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VdrawMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VdrawResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new VdrawSpec([]),
            'result' => new VdrawResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = VdrawMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new VdrawError('boom', 'boom');
        $utility->fetcher = function (VdrawContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VdrawSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = VdrawMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (VdrawContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = VdrawMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(VdrawResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (VdrawContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VdrawSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = VdrawMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new VdrawResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new VdrawSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = VdrawMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = VdrawMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = VdrawMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new VdrawResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, VdrawDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new VdrawResult(['ok' => false])]);
        $this->expectException(VdrawError::class);
        VdrawDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new VdrawResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', VdrawMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new VdrawBaseFeature();
        $b = new VdrawBaseFeature();
        VdrawFeatureAdd::call($ctx, $a);
        VdrawFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): VdrawBaseFeature
    {
        $f = new VdrawBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): VdrawContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new VdrawSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new VdrawResponse(['headers' => null]),
            'result' => new VdrawResult([]),
        ]);
        VdrawResultHeaders::call($ctx);
