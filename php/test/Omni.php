...
//    only - a typed VdrawContext there would make every ctx
...
final class VdrawOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new VdrawContext($clean, null);
    ...
    public static function err_from_map($m): ?VdrawError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new VdrawError(is_string($code) ? $code : '', $msg);
    }
