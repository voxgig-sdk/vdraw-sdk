...
class VdrawSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = VdrawHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = VdrawHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!VdrawFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, VdrawFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return VdrawUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = VdrawHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = VdrawHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = VdrawHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new VdrawError($op . "_allow",
                "VdrawSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = VdrawHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new VdrawError("graphql_error",
                "VdrawSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function UsernameGeneration($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_username_generation === null) {
                $this->_username_generation = new UsernameGenerationEntity($this, null);
            }
            return $this->_username_generation;
        }
        return new UsernameGenerationEntity($this, $data);
    }
