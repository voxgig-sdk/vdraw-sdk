...
class VdrawNetsimFeature extends VdrawBaseFeature
{
    ...
    private function _simulate(VdrawContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(VdrawContext $ctx, array $applied): void
    {
        $client = $this->client;
