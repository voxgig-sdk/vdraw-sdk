...
class FtCtrl extends VdrawControl
{
...
class FtClient
{
    ...
    private VdrawUtility $utility;

    public function __construct(VdrawUtility $utility, array $options)
    {
    ...
    public function get_utility(): VdrawUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (VdrawContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (VdrawContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = VdrawConfig::make_feature($name);
    ...
    private static function buildUrl(VdrawSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof VdrawError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new VdrawResponse($fetched);
        }
    ...
    private function fail(VdrawContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (VdrawContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
