...
type Args struct {
	Entity string         `json:"entity" jsonschema:"username_generation"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.VdrawSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "vdraw_list",
		Description: "List records from Vdraw. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "vdraw_load",
		Description: "Load a single record from Vdraw. " +
...
func runOp(client *sdk.VdrawSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.VdrawSDK, name string) (sdk.VdrawEntity, error) {
	switch strings.ToLower(name) {
	case "username_generation":
		return client.UsernameGeneration(nil), nil
