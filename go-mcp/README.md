...
MCP server exposing the Vdraw SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `vdraw_list` | `entity`, optional `query` map | First page of records as JSON |
| `vdraw_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `vdraw_list` and
`vdraw_load` tools become available in new sessions.
