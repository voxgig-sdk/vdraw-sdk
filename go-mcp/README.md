...
[MCP](https://modelcontextprotocol.io) server exposing the Vdraw SDK as
two agent tools — `vdraw_list` and `vdraw_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vdraw-mcp)
make build

# 2. Provide credentials via the environment
export VDRAW_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user vdraw \
  -- /absolute/path/to/vdraw-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./vdraw-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// vdraw_list: first page of records
{ "entity": "username_generation" }
{ "entity": "username_generation", "query": { } }

// vdraw_load: one record by id
{ "entity": "username_generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vdraw-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user vdraw \
     -- "$PWD"/dist/*/vdraw-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `vdraw_list` and `vdraw_load` tools now appear
   in new sessions. Ask the agent to *"list username_generation using vdraw"*
   and it calls `vdraw_list` with `{"entity":"username_generation"}`.
...
```sh
export VDRAW_APIKEY=sk_live_xxx            # API key
export VDRAW_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `vdraw_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `vdraw_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VDRAW_APIKEY` | API key sent with every request. |
| `VDRAW_BASE` | Optional override of the API base URL. |
...
```sh
./vdraw-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vdraw_load","arguments":{"entity":"username_generation","query":{"id":1}}}}'
```
