...
class VdrawTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VdrawTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VdrawTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VdrawTestRunner.getenv("VDRAW_TEST_LIVE")
        override = VdrawTestRunner.getenv("VDRAW_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VdrawTestRunner.getenv(key)
        ...
        explain = VdrawTestRunner.getenv("VDRAW_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VDRAW_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if VdrawTestRunner._test_control is not None:
            return VdrawTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VdrawTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VdrawTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VdrawTestRunner.load_test_control()
...
def load_env_local():
    VdrawTestRunner.load_env_local()


def env_override(m):
    return VdrawTestRunner.env_override(m)


def entity_data(v):
    return VdrawTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VdrawTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VdrawTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VdrawTestRunner.load_test_control()


def live_client_options():
    return VdrawTestRunner.live_client_options()


def live_delay_ms():
    return VdrawTestRunner.live_delay_ms()
