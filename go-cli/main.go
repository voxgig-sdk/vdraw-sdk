// Command Vdraw-cli is a CLI + REPL for the Vdraw SDK,
...
const entitiesHelp = "username_generation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      VDRAW_APIKEY (api key), VDRAW_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: VDRAW_APIKEY carries the API key and
	// VDRAW_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("VDRAW_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("VDRAW_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewVdrawSDK(opts)
