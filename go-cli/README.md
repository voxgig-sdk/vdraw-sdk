...
boru-driven command-line client **and** interactive REPL for the Vdraw
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vdraw-cli)
make build
...
VDRAW_BASE=https://api.example.com ./vdraw-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vdraw-cli
   ```
...
Run with no arguments to open a REPL (prompt `vdraw>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `username_generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/vdraw-cli`. |
...
(`./vdraw-cli <expr>`) and interactively (the REPL), and expressions compose the
